...
void DualSense_Input::evaluateDualSenseHidInputBuffer(uint8_t *_reportData) { 
    ...
    DS_input.leftStick.x = (_reportData[2] << 4) / 4080.;
    DS_input.leftStick.y = 1. - (_reportData[3] << 4) / 4080.;
    DS_input.rightStick.x = (_reportData[4] << 4) / 4080.;
    DS_input.rightStick.y = 1. - (_reportData[5] << 4) / 4080.;
   
    // Buttons
    uint8_t buttonData = _reportData[9];
    ...
    DS_input.buttons.touchPad = (_reportData[11] & DualSense_ISTATE_BTN_B_PAD_BUTTON) != 0;
    
    
    // Convert trigger to unsigned range, BT 0x09
    DS_input.leftTrigger = (_reportData[6] << 4) / 4080.;
    DS_input.rightTrigger = (_reportData[7] << 4) / 4080.;
    

    //gyo // Gyroscope data is relative (degrees/s)
    DS_input.gyroscope.x = (float)uint16_to_int16(_reportData[17] | ((_reportData[18] << 8) & 0xFF00)) * (2000.0f / 32767.0f);
    DS_input.gyroscope.y = (float)uint16_to_int16(_reportData[19] | ((_reportData[20] << 8) & 0xFF00)) * (2000.0f / 32767.0f);
    DS_input.gyroscope.z = (float)uint16_to_int16(_reportData[21] | ((_reportData[22] << 8) & 0xFF00)) * (2000.0f / 32767.0f);
    
    //accelerometer // convert to real units
    DS_input.accelerometer.x = (float)uint16_to_int16(_reportData[23] | ((_reportData[24] << 8) & 0xFF00)) / 8192.0f;
    DS_input.accelerometer.y = (float)uint16_to_int16(_reportData[25] | ((_reportData[26] << 8) & 0xFF00)) / 8192.0f;
    DS_input.accelerometer.z = (float)uint16_to_int16(_reportData[27] | ((_reportData[28] << 8) & 0xFF00)) / 8192.0f;
    ...
    //TouchPoint 2 points
    DS_input.touchPoint1.id = _reportData[34] & 0x7F;
    DS_input.touchPoint2.id = _reportData[38] & 0x7F;
    
    DS_input.touchPoint1.down = (_reportData[34]& 0x80) == 0;
